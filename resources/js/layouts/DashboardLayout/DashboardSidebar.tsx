...
const AppSidebar: React.FC = () => {
        ...
        <li key={nav.label}>
          {nav.subItems ? (
            ...
            nav.path && (
              <Link
                to={nav.path}
                className={`menu-item group ${isActive(nav.path) ? "text-primary bg-white" : "text-white"
                  }`}
              >
                <span
                  className={`menu-item-icon-size`}
              ...
              <ul className="mt-2 space-y-1 ml-9">
                {nav.subItems.map((subItem: any) => (
                  <li key={subItem.label}>
                    <Link
                      to={subItem.path}
                      className={`menu-dropdown-item opacity-80 ${isActive(subItem.path)
                        ? "text-primary bg-white"
                        : "text-white"
                        }`}
                    >
  ...
  return (
    <aside
      className={`fixed mt-16 flex flex-col lg:mt-0 top-0 px-5 left-0 bg-steel-gray dark:bg-gray-900 dark:border-gray-800 text-white h-screen transition-all duration-300 ease-in-out z-10 shadow-[5px_0px_10px_rgba(0,0,0,0.3)] 
      ...
      >
        <Link to="/">
          <div className="flex gap-4 items-center">
            ...
            />
            {(isExpanded || isHovered || isMobileOpen) &&
              <h4 className="font-[800] text-xl">{appName}</h4>
            }
          </div>
