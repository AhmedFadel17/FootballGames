...
export default function TeamCard({
    ...
    const { id, name, abbr, popularity, img_src, titles_won, country } = team;
        ...
        <div className="glass-card rounded-2xl overflow-hidden group border border-white/10 hover:border-accent-cyan/40 transition-all duration-300 flex flex-col justify-between relative bg-dashboard-bg/50 backdrop-blur-md">
            ...
            >
                ...
                {img_src ? (
                    <img
                        src={img_src}
                        alt={name}
                        className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 opacity-25 blur-xs"
                    />
                ) : (
                ...
                {abbr && (
                    <div className="absolute top-3 right-3 px-2 py-0.5 rounded-md bg-white/10 backdrop-blur-md border border-white/10 text-[10px] font-black tracking-wider text-accent-cyan uppercase">
                        {abbr}
                    </div>
                )}
                ...
                <div className="absolute bottom-3 left-4 right-4 flex items-center gap-3">
                    ...
                    <div className="w-12 h-12 rounded-xl bg-white/10 backdrop-blur-md p-1.5 shrink-0 border border-white/10 flex items-center justify-center shadow-lg">
                        {img_src ? (
                            ...
                            <span className="material-symbols-outlined text-white/40 text-2xl">shield</span>
                        )}
                    </div>
            ...
            <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="bg-white/5 rounded-xl p-2.5 border border-white/5 flex flex-col justify-center">
                        ...
                        <div className="flex items-center gap-2">
                            <div className="text-sm font-bold text-white">
                                {popularity ?? 0}<span className="text-xs text-white/40">/100</span>
                            </div>
                    ...
                    {/* Titles Won Stat */}
                    <div className="bg-white/5 rounded-xl p-2.5 border border-white/5 flex flex-col justify-center">
                        <div className="flex items-center justify-between text-[10px] uppercase font-bold text-amber-400/80 tracking-wider mb-1">
                            <span>Titles Won</span>
                            <span className="material-symbols-outlined text-xs text-amber-400">emoji_events</span>
                        </div>
                        <div className="text-sm font-bold text-white">
                            {titles_won ?? 0} <span className="text-xs font-normal text-white/40">Trophies</span>
                        </div>
                    </div>
