...
export default function ManagerCard({
    ...
    const { id, name, popularity, img_src, country, team_periods } = manager;

    // Calculate total team periods / clubs managed
    const totalClubs = team_periods?.length ?? 0;
                ...
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="bg-white/5 rounded-xl p-2.5 border border-white/5 flex flex-col justify-center">
                        <div className="flex items-center justify-between text-[10px] uppercase font-bold text-amber-400/80 tracking-wider mb-1">
                            <span>Spells / Teams</span>
                            <span className="material-symbols-outlined text-xs text-amber-400">groups</span>
                        </div>
                        <div className="text-xs font-bold text-white">
                            {totalClubs} <span className="text-[10px] font-normal text-white/40">{totalClubs === 1 ? 'Period' : 'Periods'}</span>
                        </div>
                    </div>
